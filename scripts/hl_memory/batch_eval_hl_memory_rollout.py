...
def main() -> None:
    ...
    jobs = _build_jobs(args, passthrough=passthrough)
    if not jobs:
        raise FileNotFoundError(f"No dataset dirs found under {args.dataset_root} matching {args.dataset_glob!r}")
    print(f"[Info] tasks={len(jobs)} workers={args.workers} output_root={args.output_root}", flush=True)
    ...
    if args.dry_run:
        ...
        with ThreadPoolExecutor(max_workers=args.workers) as pool:
            future_to_job = {}
            for job in jobs:
                print(f"[Queue] {job.task_id} dataset={job.dataset_dir} log={job.log_path}", flush=True)
...
def _build_jobs(args: argparse.Namespace, *, passthrough: list[str]) -> list[Job]:
    ...
    for dataset_dir in dataset_dirs:
        ...
        jobs.append(Job(task_id=task_id, dataset_dir=dataset_dir, output_json=output_json, log_path=log_path, cmd=cmd))
    return jobs
