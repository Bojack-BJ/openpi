...
- `subtask_progress`: scalar in `[0, 1]` estimating completion of the current objective. It is generated from segment start/end and frame index during offline normalization/export.
- `should_advance_objective`: boolean hint for whether the current objective appears complete enough to transition. It is generated by rules from progress or success events.
...
subtask_segments.json
  ...
  -> scripts/hl_memory/normalize_hl_annotations_with_llm.py
  -> hl_annotations_llm_normalized.jsonl
  -> scripts/hl_memory/export_hl_memory_dataset.py
  -> samples.jsonl
```

`scripts/hl_memory/export_hl_memory_dataset.py` should consume the normalized annotation JSONL for training data. It reads `task_progress`, `current_objective`, `subtask_progress`, `should_advance_objective`, `active_hand`, `relevant_objects`, and `notes` from each annotation row when present. If those fields are absent, it falls back to deterministic rule-based rendering from `current_subtask`, `phase`, `target_query`, `goal_query`, and event metadata, but that fallback is mainly for debugging.
...
- The prompt asks for `task_progress`, `current_objective`, `relevant_objects`, `notes`, `keyframe_candidate_positions`, `phase`, `target_query`, `goal_query`, optional `subtask_progress`, `should_advance_objective`, `active_hand`, optional SAM fields, and optional `target_bbox_xyxy`.
