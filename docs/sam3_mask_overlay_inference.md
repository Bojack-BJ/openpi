...
The server replaces `obs["image"][view]` with the overlay image before calling the policy, and attaches the latest overlay payload to the policy response for debugging. If `text_select_video` loses the selected instance, the server returns only `__mask_overlay` with `needs_reprompt=True`; the rollout client pauses policy execution, opens the click window on the current frame, resets the tracker, and then continues.
