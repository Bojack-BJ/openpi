...
QWEN2_5_TEMPORAL_PATCH_SIZE = 2
QWEN2_5_SPATIAL_MERGE_SIZE = 2
QWEN2_5_VISION_HIDDEN_SIZE = 1280
QWEN2_5_VISION_DEPTH = 32
QWEN2_5_VISION_NUM_HEADS = 16
QWEN2_5_VISION_MLP_DIM = 3420
QWEN2_5_VISION_NUM_POSITIONS = 2304
QWEN2_5_VISION_MERGER_DIM = 3420
...
def _resize_positional_embedding(positional_embedding: jax.Array, *, grid_h: int, grid_w: int) -> jax.Array:
    ...
    if source_side * source_side != source_positions:
        raise ValueError(f"Qwen2.5 vision pos embedding expects a square grid, got {source_positions} positions.")
...
def _vision_rotary_cache(
    ...
    if head_dim % 4 != 0:
        raise ValueError(f"Qwen2.5 vision rotary expects head_dim divisible by 4, got {head_dim}")
...
@at.typecheck
class Qwen2_5VisionPatchEmbed(nn.Module):
    hidden_size: int
    patch_size: int
...
class Qwen2_5VisionPatchMerger(nn.Module):
    ...
    def __call__(self, image_tokens: at.Float[at.Array, "b s d"], *, grid_h: int, grid_w: int):
        if grid_h % self.merge_size != 0 or grid_w % self.merge_size != 0:
            raise ValueError(
                "Qwen2.5 vision merger requires the spatial grid to be divisible by the merge size: "
        ...
        image_tokens = nn.silu(image_tokens)
        image_tokens = nn.Dense(
            self.output_width,
            use_bias=True,
            name="linear_fc2",
...
class _Qwen2_5VisionModule(nn.Module):
    output_width: int
    hidden_size: int
    depth: int
    mlp_dim: int
    ...
    def __call__(self, image: at.Float[at.Array, "b h w c"], *, train: bool = False):
        del train
        patches = Qwen2_5VisionPatchEmbed(
        ...
        merged_tokens, merged_grid_thw = Qwen2_5VisionPatchMerger(
...
class Qwen2_5VisionTower(nnx.Module):
    """Official-style JAX Qwen2.5 vision tower."""
    ...
    def __init__(
        ...
        *,
        ...
        image_example,
        rngs: nnx.Rngs,
        hidden_size: int = QWEN2_5_VISION_HIDDEN_SIZE,
        depth: int = QWEN2_5_VISION_DEPTH,
        mlp_dim: int = QWEN2_5_VISION_MLP_DIM,
        num_heads: int = QWEN2_5_VISION_NUM_HEADS,
        patch_size: int = QWEN2_5_VISION_PATCH_SIZE,
        temporal_patch_size: int = QWEN2_5_TEMPORAL_PATCH_SIZE,
        num_positions: int = QWEN2_5_VISION_NUM_POSITIONS,
        spatial_merge_size: int = QWEN2_5_SPATIAL_MERGE_SIZE,
        merger_dim: int = QWEN2_5_VISION_MERGER_DIM,
    ):
        self.patch_size = patch_size
        self.temporal_patch_size = temporal_patch_size
        self.spatial_merge_size = spatial_merge_size
        self.encoder = nnx_bridge.ToNNX(
            _Qwen2_5VisionModule(
...
__all__ = [
    "QWEN2_5_SPATIAL_MERGE_SIZE",
    "QWEN2_5_TEMPORAL_PATCH_SIZE",
    "QWEN2_5_VISION_DEPTH",
    "QWEN2_5_VISION_HIDDEN_SIZE",
    "QWEN2_5_VISION_MERGER_DIM",
    "QWEN2_5_VISION_MLP_DIM",
    "QWEN2_5_VISION_NUM_HEADS",
    "QWEN2_5_VISION_NUM_POSITIONS",
    "QWEN2_5_VISION_PATCH_SIZE",
    "Qwen2_5VisionTower",
]
